...
//        ;
//    }
 public function findAllGreaterThanPrice($lieu): array
    {
        $conn = $this->getEntityManager()->getConnection();

        $sql = "SELECT e.id , user_id, nobrchambresoffertes, lieu_id, nom,rue, cp, ville 
        FROM  etablissement e, lieu l 
        WHERE e.lieu_id= l.id 
        and l.ville=:lieu";
        $stmt = $conn->prepare($sql);
        $resultSet = $stmt->executeQuery(['lieu' => $lieu]);
